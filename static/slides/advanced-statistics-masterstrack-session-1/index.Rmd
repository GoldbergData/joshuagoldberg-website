---
title: "Statistical Thinking for Machine Learning"
subtitle: ""
author: "Joshua Goldberg <br><span style='font-size: 50%;'>UChicago MastersTrack: Coursera"
output:
...
- .left[Hypothesis testing]
  - .left[Testing multiple parameters are once — T test vs. F test, Testing equality of two parameters]
  - .left[Testing equality of two parameters]
...
1. Population model (true relationship): $y=\beta_0+\beta_1x_1+\beta_2x_2+\epsilon$

2. Our model: $y=\hat{\beta}_0+\hat{\beta}_1x_1+\epsilon$
...
- Fit a model with all three: $\text{salary}=9226.169\times \text{HR}+225.884 \times \text{RBI}+2.982 \times\text{BA}$
- What is this model saying? Why not: $\text{salary}=9934.27\times \text{HR}+3291.02 \times \text{RBI}+1002.95 \times\text{BA}$
...
- Recall the T test for $H_0 \rightarrow \beta_1=\theta\text{:} \frac{(\beta_1-\theta)}{\text{SE}(\beta_1)}$
...
- How should we test $\beta_1=\beta_2$?
- $H_0\rightarrow \beta_1=\beta_2$ is erquivalent to $\beta_1-\beta_2=0$
- Before, when $H_0\rightarrow \beta_1=0, T=\frac{\beta_1}{\sqrt{\text{Var}(\beta_1)}}$
- What is $\text{var}(\beta_1-\beta_2)$?
  - $\text{Var}(\beta_1)+\text{Var}(\beta_2)-2 \times \text{Cov}(\beta_1, \beta_2)$
...
$\text{ln}(ax)=\text{ln}+\text{ln}(x)$<br><br>
]
.pull-right[
...
- Log-level: $\text{ln}y=5+0.2(x)$
  - 1 unit change in $x=\beta_1 * 100\text{%}$ change in $y$
<br><br>
