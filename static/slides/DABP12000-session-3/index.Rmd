...
output:
  xaringan::moon_reader:
    css: [default, css/uchicago_custom.css, robot-fonts]
    lib_dir: libs
    nature:
...
con_mydb <- dbConnect(
  MySQL(),
  user = key_list("sql")$username,
  password = key_get("sql"),
  dbname = "mydb",
  host = "localhost"
)
```
...
R can augment your SQL efforts by generating queries with dbplyr and by passing data between R and SQL. But that's not it. R can help you reuse SQL code by [placing SQL code in R functions](https://glue.tidyverse.org/reference/glue_sql.html). Say you want reuse a query to pull sales data and you are tired of copying/pasting. You can do something like this: 

```{r eval=FALSE}
query_sales_data <- function(state, city, con_mydb) {
  query <- glue_sql("SELECT * 
           FROM sales 
           WHERE state = {state}", .con = con_mydb)
  DBI::dbGetQuery(query)
}
```

I have not formerly introduced this concept yet, but I wanted to expose the idea here to show the possibilities when using R and SQL. 
